  ...
  async updateEntitiesOfType<TEntity, TInput, TMainEntity>(
    ...
    for (const [inputIndex, entityToSave] of entitiesToSave.entries()) {
      ...
      if (modelUpdater.entityBatchMode) {
        ...
        const singleUpdater = modelUpdater as IRapidModelUpdaterSingleGetMode<TEntity, TInput, any>;
        entityInRapid = await singleUpdater.entitySingleFinder(singleUpdater.entityKeyGetter(entityToSave));
      }
